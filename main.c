...
void exibir_menu(void) {
    ...
    printf("4 - Exibir total a prazo (com juros aplicado)\n");
    printf("0 - Sair\n");
...
void listar_produtos(Produto lista[], int total) {
    ...
    for (int i = 0; i < total; i++) {
        printf("ID: %d | Codigo de Barras: %s | Nome: %s | Preco: R$ %.2f | Qtd: %d", lista[i].id, lista[i].codigo_barras, lista[i].nome, lista[i].preco, lista[i].quantidade);
    }
}
    ...
    while (opcao != 0) {
        ...
        switch (opcao) {
            ...
            case 2:
                ...
                break;
            case 4:
                printf("\nTotal a prazo (Com juros aplicado): R$ %.2f\n",
